...
const Dashboard = () => {
  ...
  const processForecastData = (forecastData) => {
    ...
    const groupedByDay = forecastData.list.reduce((acc, item) => {
      ...
      if (date !== today) { // Skip today as we have current weather
        if (!acc[date]) {
          acc[date] = [];
        }
        acc[date].push(item);
    ...
    // Get the next 4 days
    Object.keys(groupedByDay).slice(0, 4).forEach(date => {
              ...
              {dailyForecasts.map((forecast, index) => (
                <div key={index} className="flex items-center justify-between bg-white/10 rounded-lg p-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="font-medium">{formatDate(forecast.date)}</div>
